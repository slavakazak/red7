...
export default function checkRule(players, ruleCard) {
  ...
  newPlayers.forEach(player => {
    ...
    player.hand.forEach(card => card.active = false)
    player.palette.forEach(card => card.active = false)
  })
  ...
  newPlayers.forEach(player => {
    if (player.palette.length) {
      ...
      if (ruleCard.color.value === 1) { //Больше всего карт номиналом меньше 4
        player.palette = player.palette.map(a => ({ ...a, active: a.value < 4 }))
        ...
        maxOrder.forEach(card => card.active = true)
        ...
        diffСolors.forEach(card => card.active = true)
      } else if (ruleCard.color.value === 4) { //Больше всего чётных карт
        player.palette = player.palette.map(a => ({ ...a, active: a.value % 2 === 0 }))
        ...
        maxSameColors.forEach(card => card.active = true)
        ...
        maxSame.forEach(card => card.active = true)
      } else { //Старшая карта
        player.palette[0].active = true
      }
    }
  })
  ...
  newPlayers.forEach(player => {
    player.palette.forEach(card => {
      if (card.active) player.numOfWinCards++
      if (card.active && !player.maxWinCard) player.maxWinCard = card
    })
  })
  //поиск победителя
  let winner = newPlayers.find(player => !player.loss)
  for (let i = 0; i < newPlayers.length; i++) {
    const player = newPlayers[i]
    if (player.loss) continue
    if (player.numOfWinCards < winner.numOfWinCards) continue
    if (player.numOfWinCards === winner.numOfWinCards) {
      const playerMaxWinCardValue = player.maxWinCard ? player.maxWinCard.value : 0
      const winnerMaxWinCardValue = winner.maxWinCard ? winner.maxWinCard.value : 0
      const playerMaxWinCardColorValue = player.maxWinCard ? player.maxWinCard.color.value : 0
      const winnerMaxWinCardColorValue = winner.maxWinCard ? winner.maxWinCard.color.value : 0
      if (playerMaxWinCardValue < winnerMaxWinCardValue) continue
      if (playerMaxWinCardValue === winnerMaxWinCardValue && playerMaxWinCardColorValue < winnerMaxWinCardColorValue) continue
    }
    winner = player
  }
  winner.win = true
